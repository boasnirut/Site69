...
export function ActivitySlider({ variant = "home", initialActivities = [] }: ActivitySliderProps) {
  const [activeIndex, setActiveIndex] = useState(Math.min(4, Math.max(0, initialActivities.length - 1)));
  ...
  return (
    <section className={isPage ? "section-block activity-accordion-section page" : "section-block activity-accordion-section"}>
      ...
      <div className="activity-accordion" aria-label="สไลด์ภาพกิจกรรมแบบ accordion">
        {initialActivities.map((item: any, index: number) => {
